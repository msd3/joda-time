 ...
 * <p>
 * A duration is defined by a fixed number of milliseconds. There is no concept
 * of fields, such as days or seconds, as these fields can vary in length.
 * <p>
 * A duration may be converted to a {@link Period} to obtain field values. This
...
public final class Duration
        ...
        implements ReadableDuration, Serializable, java.time.temporal.TemporalAmount {
    ...
    * 
    */
   public List<TemporalUnit> supportedUnits = Collections.unmodifiableList(
           Arrays.asList(
                   ChronoUnit.MILLIS, 
                   ChronoUnit.SECONDS, 
                   ChronoUnit.MINUTES, 
    ...
    * <p>
    * This method assumes that there are 24 hours in a day, 60 minutes in an
    * hour, 60 seconds in a minute and 1000 milliseconds in a second. This will
    * be true for most days, however days with Daylight Savings changes will not
    * have 24 hours, so use this method with care.
    * <p>
    * A Duration is a representation of an amount of time. If you want to
    * express the concept of 'days' you should consider using the {@link Days}
    * class.
    *
    ...
    * <p>
    * This method assumes that there are 60 minutes in an hour, 60 seconds in a
    * minute and 1000 milliseconds in a second. All currently supplied
    * chronologies use this definition.
    * <p>
    * A Duration is a representation of an amount of time. If you want to
    * express the concept of 'hours' you should consider using the {@link Hours}
    * class.
    *
    ...
    * <p>
    * This method assumes that there are 60 seconds in a minute and 1000
    * milliseconds in a second. All currently supplied chronologies use this
    * definition.
    * <p>
    * A Duration is a representation of an amount of time. If you want to
    * express the concept of 'minutes' you should consider using the
    * {@link Minutes} class.
    *
    ...
    * <p>
    * This method assumes that there are 1000 milliseconds in a second. All
    * currently supplied chronologies use this definition.
    * <p>
    * A Duration is a representation of an amount of time. If you want to
    * express the concept of 'seconds' you should consider using the
    * {@link Seconds} class.
    *
    ...
    * have 24 hours, so use this method with care.
    * <p>
    * This returns <code>getMillis() / MILLIS_PER_DAY</code>. The result is an
    * integer division, thus excess milliseconds are truncated.
    *
    ...
    * chronologies use this definition.
    * <p>
    * This returns <code>getMillis() / MILLIS_PER_HOUR</code>. The result is an
    * integer division, thus excess milliseconds are truncated.
    *
    ...
    * definition.
    * <p>
    * This returns <code>getMillis() / MILLIS_PER_MINUTE</code>. The result is
    * an integer division, thus excess milliseconds are truncated.
    *
    ...
    * currently supplied chronologies use this definition.
    * <p>
    * This returns <code>getMillis() / 1000</code>. The result is an integer
    * division, so 2999 millis returns 2 seconds.
    *
   ...
   //-----------------------------------------------------------------------
   /**
    * Get this duration as an immutable <code>Duration</code> object by
    ...
    * @return
    */
   public java.time.Duration toJavaNetDuration() {
      java.time.Duration javaNetDuration = java.time.Duration.ofMillis(this.getMillis());
    ...
    * never null
    * @throws ArithmeticException if the number of days is too large to be
    * represented
    * @since 2.0
    */
    ...
    * definition.
    *
    * @return a period representing the number of standard minutes in this
    * period, never null
    ...
    * currently supplied chronologies use this definition.
    *
    * @return a period representing the number of standard seconds in this
    * period, never null
   ...
   /**
    * Returns a new duration with this length plus that specified multiplied by
    * the scalar. This instance is immutable and is not altered.
    * <p>
   ...
   //-----------------------------------------------------------------------
   /**
    * Returns a new duration with this length plus that specified. This instance
    * is immutable and is not altered.
    * <p>
    * If the addition is zero, this instance is returned.
    *
   ...
   /**
    * Returns a new duration with this length multiplied by the specified
    * multiplicand. This instance is immutable and is not altered.
    * <p>
    ...
    * {@code RoundingMode} can be specified. This instance is immutable and is
    * not altered.
    * <p>
    * If the divisor is one, this instance is returned.
    *
   ...
   /**
    * Returns a new duration with this length negated. This instance is
    * immutable and is not altered.
    *
    * @return the new duration instance
    */
   ...
   public long get(TemporalUnit unit) {
      if (unit == ChronoUnit.MILLIS) {
         ...
         return this.getStandardSeconds();
      } else if (unit == ChronoUnit.MINUTES) {
         return this.getStandardMinutes();
      } else if (unit == ChronoUnit.HOURS) {
         return this.getStandardHours();
      } else if (unit == ChronoUnit.DAYS) {
         return this.getStandardDays();
      } else {
   ...
   public Temporal addTo(Temporal temporal) {
      return temporal.plus(getMillis(), ChronoUnit.MILLIS);
   }

   @Override
   public Temporal subtractFrom(Temporal temporal) {
      return temporal.minus(getMillis(), ChronoUnit.MILLIS);
   }
